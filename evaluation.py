...
def get_y_true(task_name):
    ...
    if task_name in ["sentihood_single", "sentihood_NLI_M", "sentihood_QA_M", "sentihood_NLI_B", "sentihood_QA_B"]:
        true_data_file = "absa_data/sentihood/bert-pair/test_NLI_M.tsv"
...
def main():
    ...
    pred_data_files = []
    if os.path.isdir(args.pred_data_dir):
        for root, ds, fs in os.walk(args.pred_data_dir):
            for f in fs:
                fullname = os.path.join(root, f)
                if 'test_ep_' in fullname:
                    pred_data_files.append(fullname)
    ...
    for index, f in enumerate(pred_data_files):
        ...
        if 'sentihood_NLI_M' in f:
            args.task_name = 'sentihood_NLI_M'
        elif 'sentihood_QA_M' in f:
            args.task_name = 'sentihood_QA_M'
        elif 'sentihood_NLI_B' in f:
            args.task_name = 'sentihood_NLI_B'
        elif 'sentihood_QA_B' in f:
            args.task_name = 'sentihood_QA_B'
        elif 'semeval_NLI_M' in f:
            args.task_name = 'semeval_NLI_M'
        elif 'semeval_QA_M' in f:
            args.task_name = 'semeval_QA_M'
        elif 'semeval_NLI_B' in f:
            args.task_name = 'semeval_NLI_B'
        elif 'semeval_QA_B' in f:
            args.task_name = 'semeval_QA_B'

        print(index, args.task_name, f)
        ...
        try:
            ...
            if args.task_name in ["sentihood_single", "sentihood_NLI_M", "sentihood_QA_M", "sentihood_NLI_B", "sentihood_QA_B"]:
                ...
                y_pred, score = get_y_pred(args.task_name, f)
            ...
            for key in result.keys():
                ...
                if result[key] > result_max[key]:
                    result_max[key] = result[key]
                    result_max_files[f] = result
        except Exception as e:
            print(e)
            traceback.print_exc()

    for k, v in result_max_files.items():
        print(k)
        for key in v.keys():
            print(key, "=",str(v[key]))
    print("best_num:\n", result_max)
