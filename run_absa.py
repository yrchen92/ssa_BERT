...
def main(args): 
    ...
    if args.do_first_eval:
        ...
        for key in sorted(test_res.keys()):
            logger.info("first test:  %s = %s", key, str(eval_res[key]))
        ...
        if task_name == "mnli":
            ...
            idx, preds, test_res = do_test(args, label_list, task_name, processor, tokenizer, output_mode, model, do_mm=True)
            ...
            if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0) and epoch % 1 == 0:
                ...
                if tmp_acc >= best_val_acc:
                    ...
                    if args.do_test:
                        ...
                        if "mcc" in test_res:
                            result['test_mcc'] = test_res["mcc"]
                        if "corr" in test_res:
                            result['test_corr'] = test_res["corr"]
                        if "f1" in test_res:
                            result['test_f1'] = test_res['f1']
                        if "spearmanr" in test_res:
                            result['test_spearmanr'] = test_res['spearmanr']
                        if "pearson" in test_res:
                            result['test_pearson'] = test_res['pearson']
                        if "acc_and_f1" in test_res:
                            result['test_acc_and_f1'] = test_res['acc_and_f1']
                        

                        logger.info("  idx_num = %d  preds_num = %d", idx, len(preds))
                        ...
                        if task_name == "mnli":
                            ...
                            idx, preds, test_res = do_test(args, label_list, task_name, processor, tokenizer, output_mode, model, do_mm=True)
                            if "acc" in test_res:
                                result['test_mm_acc'] = test_res["acc"]
                            if "mcc" in test_res:
                                result['test_mm_mcc'] = test_res["mcc"]
                            if "corr" in test_res:
                                result['test_mm_corr'] = test_res["corr"]
                            if "f1" in test_res:
                                result['test_mm_f1'] = test_res['f1']
                ...
                output_eval_file = os.path.join(args.output_dir, dev_test + "_results_" + str(tmp_acc) + ".txt")
